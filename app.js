...
app.post("/login", async (request, response) => {
  ...
  if (dbUser === undefined) {
    response.status(400).set("Content-Type", "text/plain").send("Invalid user");
  } else {
...
app.put("/change-password", async (request, response) => {
  ...
  const checkFroUserQuery = `SELECT * FROM user WHERE username = '${username}';`;
  const dbUser = await database.get(checkFroUserQuery);
  if (dbUser === undefined) {
    response.status(400);
    ...
    if (isValidPassword === true) {
      ...
      if (lengthOfNewPassword < 5) {
        ...
        const updatedPasswordQuery = `UPDATE user SET password= '${encryptedPassword}' WHERE username= '${username}';`;
